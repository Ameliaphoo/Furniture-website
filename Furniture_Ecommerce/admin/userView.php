<?php 
	...
	if(isset($_POST['confirm_delete'])) {
		...
		header("Location: userView.php?delete=ok");
		exit;
	}
?>
	<div class="px-5">
		<div class="cardHeader">
			<h1 class="admin_title"> Users List</h1>
			...
			<div class="max-w-full overflow-x-auto">
				...
				<thead class=" border-y-2 ">
					<tr class="text-center  bg-gray-2 dark:bg-meta-4">
						...
						<td class="py-3 text-center">Username</td>
						<td class="py-3 text-center">Email</td>
						<td class="py-3 text-center">Phone Number</td>
						<td class="py-3 text-center">Address</td>
						<td class="py-3 text-center">Type</td>
				...
				<tbody>
					...
					<tr>
						<td class="border-b text-center border-[#eee] py-5  dark:border-strokedark">
							<h5 class="font-medium text-black  "><?php echo $user['uid']; ?></h5>
						</td>
						<td class="border-b text-center border-[#eee] py-5  dark:border-strokedark">
						    <h5 class="font-medium text-black  "><?php echo $user['uname']; ?></h5>
						</td>
						<td class="border-b text-center border-[#eee] py-5  dark:border-strokedark">
						    <h5 class="font-medium text-black  "><?php echo $user['uemail']; ?></h5>
						</td><td class="border-b text-center border-[#eee] py-5  dark:border-strokedark">
						    <h5 class="font-medium text-black  "><?php echo $user['uph']; ?></h5>
						</td><td class="border-b text-center border-[#eee] py-5  dark:border-strokedark">
						    <h5 class="font-medium text-black  "><?php echo $user['uaddress']; ?></h5>
						</td><td class="border-b text-center border-[#eee] py-5  dark:border-strokedark">
						    <h5 class="font-medium text-black  "><?php echo $user['utype']; ?></h5>
						</td>
						<td class=" border-b border-[#eee] py-5  dark:border-strokedark">
							<div class="flex justify-center items-center space-x-3.5">
								<a href="userUpdate.php?id=<?=  $user['uid'];  ?>" class="hover:text-amber-600">
								...
								<!-- <form method="POST"> -->
								<input type="text" hidden name="id" value="<?php echo  $user['uid'];  ?>">
								<button name="delete" class="hover:text-red-700" onclick="setAndToggleAlert(<?php echo $user['uid']; ?>)">
...
function toggleAlert(alertId, action) {
	...
	<?php
		...
		if (isset($_GET['delete'])) {
            ?>
                toastr.error('User Delete Successfull', {
                    closeButton: true,
                    progressBar: true
                });
            <?php
